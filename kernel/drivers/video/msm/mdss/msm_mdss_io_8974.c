...
//[PLATFORM]-Add-BEGIN by TCTSZ.yaohui.zeng, 2014/08/05, set DSI LDO mode for pop8lte, old boad use dc-dc mode
#ifdef CONFIG_TCT_8X16_POP8LTE
extern bool is_old_board;
#endif
...
//[PLATFORM]-Add-BEGIN by TCTSZ.yaohui.zeng, 2014/08/05, set DSI LDO mode for pop8lte, old board use dc-dc mode
#ifdef CONFIG_TCT_8X16_POP8LTE
	if(is_old_board)
	{
		printk("This is old board,do not set DSI LDO mode!\n");
		/* Regulator ctrl 0 */
		MIPI_OUTP((temp_ctrl->phy_io.base) + 0x280, 0x07);
		/* LDO ctrl */
		MIPI_OUTP((ctrl_pdata->phy_io.base) + 0x1dc, 0x00);
	}
#endif
