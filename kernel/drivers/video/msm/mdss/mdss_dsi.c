...
//[PLATFORM]-Add-BEGIN by TCTSZ.yaohui.zeng, 2014/08/05, set DSI LDO mode for pop8lte
static int lcd_board_id_gpio;
static struct dsi_pinctrl_res lcd_board_id;
bool is_old_board = false;
//[PLATFORM]-Add-END by TCTSZ.yaohui.zeng, 2014/08/05
#endif
...
//[PLATFORM]-Add-BEGIN by TCTSZ.yaohui.zeng, 2014/08/05, set DSI LDO mode for pop8lte
#ifdef CONFIG_TCT_8X16_POP8LTE
	lcd_board_id.pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR_OR_NULL(lcd_board_id.pinctrl)) {
	pr_err("%s: failed to get pinctrl\n", __func__);
	return PTR_ERR(lcd_board_id.pinctrl);
	}

	lcd_board_id.gpio_state_active
		= pinctrl_lookup_state(lcd_board_id.pinctrl,
				"mdss_lcd_board_id_active");
	if (IS_ERR_OR_NULL(lcd_board_id.gpio_state_active))
		printk("%s: can not get default pinstate\n", __func__);

	lcd_board_id.gpio_state_suspend
		= pinctrl_lookup_state(lcd_board_id.pinctrl,
				"mdss_lcd_board_id_suspend");
	if (IS_ERR_OR_NULL(lcd_board_id.gpio_state_suspend))
		printk("%s: can not get sleep pinstate\n", __func__);

	rc = pinctrl_select_state(lcd_board_id.pinctrl,lcd_board_id.gpio_state_active);
	if(rc)
		printk("can not set lcd_board_id pins\n");
#endif
//[PLATFORM]-Add-END by TCTSZ.yaohui.zeng, 2014/08/05
	return 0;
}
...
//[PLATFORM]-Add-BEGIN by TCTSZ.yaohui.zeng, 2014/04/11, Add for lcd a080ean01 power control
		...
		rc = gpio_tlmm_config(GPIO_CFG(tps65640_power_en, 0,GPIO_CFG_OUTPUT,
							...
							GPIO_CFG_ENABLE);
//[PLATFORM]-Add-BEGIN by TCTSZ.yaohui.zeng, 2014/08/05, set DSI LDO mode for pop8lte
	lcd_board_id_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "qcom,platform-lcd-board-id-gpio", 0);
	if (!gpio_is_valid(lcd_board_id_gpio))
		pr_err("%s:%d, lcd_power_en gpio not specified\n",
						__func__, __LINE__);
	rc = gpio_request(lcd_board_id_gpio, "lcd_board_id_gpio");
		if (rc) {
			pr_err("request lcd power gpio failed, rc=%d\n",
			       rc);
			return -ENODEV;
		}
	rc = gpio_direction_input(lcd_board_id_gpio);
		if (rc) {
			pr_err("set_direction for lcd_board_id_gpio failed, rc=%d\n",
			       rc);
			gpio_free(lcd_board_id_gpio);
			return -ENODEV;
	}

	if(gpio_get_value(lcd_board_id_gpio))
		is_old_board = true;
	else
		is_old_board = false;

	rc = pinctrl_select_state(lcd_board_id.pinctrl,lcd_board_id.gpio_state_suspend);
	if(rc)
		pr_err("can not set lcd_board_id pins\n");
//[PLATFORM]-Add-END by TCTSZ.yaohui.zeng, 2014/08/05
#endif
//[PLATFORM]-Add-end by TCTSZ.yaohui.zeng
