...
#define QPNP_USB_CHG_STS(base)			(base + 0xB09)
...
#define QPNP_BOOT_MONIT_MAX_COUNT	0x20
#define QPNP_BOOT_MONIT_DELAY			msecs_to_jiffies(500)
...
//[PLATFORM]-Add-BEGIN by TCTSZ.leo.guo, Fix the issue of low battery voltage display  2014/09/01
...
int qpnp_pon_is_usb_valid(void)
{
	u8 chg_sts, rc;
	...
	/* Charging Status */
	rc = spmi_ext_register_readl(pon->spmi->ctrl, pon->spmi->sid,
				QPNP_USB_CHG_STS(pon->base), &chg_sts, 1);
	if (rc) {
		dev_err(&pon->spmi->dev, "Unable to read USB_CHG_STS reg\n");
	}

	return (chg_sts & USB_IN_VALID_MASK) ? 1 : 0;
}
EXPORT_SYMBOL(qpnp_pon_is_usb_valid);


int qpnp_pon_get_boot_reason(void)
{
	u8 pon_sts, rc;
	struct qpnp_pon *pon = sys_reset_dev;
	if (!pon)
	...
	return ffs(pon_sts) - 1;
}
EXPORT_SYMBOL(qpnp_pon_get_boot_reason);
#endif
//[PLATFORM]-Add-END by TCTSZ.leo.guo
/**
...
//[PLATFORM]-Add-BEGIN by chenhui, 2014/07/03, POP8LTE for" Wake up slow off charge "PR-719671
#ifdef CONFIG_TCT_8X16_POP8LTE
		if(pon_rt_sts){
//			printk("pwr press\n");
...
#endif
//[PLATFORM]-Add-END by chenhui, 2014/07/03
...
//[PLATFORM]-Add-BEGIN by TCTSZ.leo.guo, Fix the issue of low battery voltage display  2014/09/01
#ifdef CONFIG_TCT_8X16_POP8LTE
static void boot_monitor_work_func(struct work_struct *work)
{
	struct qpnp_pon *pon =
		container_of(work, struct qpnp_pon, monitor_work.work);

	if(!qpnp_pon_is_usb_valid()){
		pr_crit("USB plug-out, shutdown the device.\n");
		if(pm_power_off) pm_power_off();
	}

	if(pon->count++ < QPNP_BOOT_MONIT_MAX_COUNT){
		schedule_delayed_work(&pon->monitor_work, QPNP_BOOT_MONIT_DELAY);
	}
	return;
}
#endif
...
//[PLATFORM]-Add-BEGIN by TCTSZ.fangyou.wang, 2014/05/28, POP8LTE for press power key 10s shut down PR-689232
#ifdef CONFIG_TCT_8X16_POP8LTE
                        if (pon_ver == 3) {
#else
			if (pon_ver == PON_REV2_VALUE) {
#endif
//[PLATFORM]-Add-END by TCTSZ.fangyou.wang, 2014/05/28
...
//[PLATFORM]-Add-BEGIN by TCTSZ.leo.guo, Fix the issue of low battery voltage display  2014/09/01
#ifdef CONFIG_TCT_8X16_POP8LTE
	if((index == PON_USB_CHG) || (index == PON_DC_CHG)){
		INIT_DELAYED_WORK(&pon->monitor_work, boot_monitor_work_func);
		schedule_delayed_work(&pon->monitor_work, 0);
	}
#endif
//[PLATFORM]-Add-END by TCTSZ.leo.guo
