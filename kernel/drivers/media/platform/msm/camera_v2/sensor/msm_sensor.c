...
#include "s5k5e2_pop8lte_v4l2_tct_OTP.h"
#include "ov5670_alto45_v4l2_tct_OTP.h"
#include "ov5670_pop8lte_v4l2_tct_OTP.h"
#endif
#endif
...
static int TCT_update_OTP(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	}else if(!strcmp(s_ctrl->sensordata->sensor_name, "s5k5e2_pop8s")){
		printk("OTP: s5k5e2_pop8s\n");
		S5K5E2_POP8S_CMCC_TCT_OTP_calibration(s_ctrl->sensor_i2c_client);
	}else if(!strcmp(s_ctrl->sensordata->sensor_name, "ov5670_alto45")){
		printk("OTP: ov5670_alto45\n");
		update_otp_wb(s_ctrl->sensor_i2c_client);
	}else if(!strcmp(s_ctrl->sensordata->sensor_name, "ov5670_pop8s")){
		printk("OTP: ov5670_pop8s\n");
		OV5670_POP8S_update_otp_wb(s_ctrl->sensor_i2c_client);
	}else{
