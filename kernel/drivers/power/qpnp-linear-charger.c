...
//[PLATFORM]-Modify-BEGIN by TCTSZ.Leo.guo, 2014/06/16, added for support smb1360 version es.
				...
				pr_warn_ratelimited("SMB1360 status: maybe i2c error.\n");
				return 	DEFAULT_CAPACITY;
#else
			...
			enum power_supply_property psp, int temp_degc)
{
	...
	case POWER_SUPPLY_PROP_WARM_TEMP:
		...
		else if (!chip->bat_is_overheat && !chip->bat_is_cold)
	...
	case POWER_SUPPLY_PROP_OVERHEAT_TEMP:
		if (temp_degc <=
		(chip->cfg_warm_bat_decidegc + HYSTERISIS_DECIDEGC)) {
			pr_smb("Can't set overheat %d lower than warm %d + hysterisis %d\n",
					temp_degc,
					chip->cfg_warm_bat_decidegc,
					HYSTERISIS_DECIDEGC);
			rc = -EINVAL;
...
static void qpnp_lbc_jeita_adc_notification(enum qpnp_tm_state state, void *ctx)
{
	...
	bool bat_warm = 0, bat_cold = 0, bat_overheat = 0;
	int temp, rc;
	unsigned long flags;
	...
	if (state == ADC_TM_WARM_STATE) {
		...
		if (temp <= (chip->cfg_cold_bat_decidegc - HYSTERISIS_DECIDEGC)) {
			...
			chip->adc_param.state_request =
					ADC_TM_HIGH_LOW_THR_ENABLE;
		} else if (temp <= (chip->cfg_overheat_bat_decidegc -HYSTERISIS_DECIDEGC)) {
			/* Overheat to warm */
			bat_warm = true;
			bat_cold = false;
			bat_overheat = false;
			...
			chip->adc_param.low_temp =
					chip->cfg_warm_bat_decidegc - HYSTERISIS_DECIDEGC;
			chip->adc_param.high_temp =
					chip->cfg_overheat_bat_decidegc;
			chip->adc_param.state_request =
					ADC_TM_HIGH_LOW_THR_ENABLE;
		}
	}

	pr_smb("cold = %d, overheat = %d, warm = %d\n",
			chip->bat_is_cold ^ bat_cold, chip->bat_is_overheat ^ bat_overheat, chip->bat_is_warm ^ bat_warm);

	if (chip->bat_is_cold ^ bat_cold || chip->bat_is_overheat ^ bat_overheat || chip->bat_is_warm ^ bat_warm) {
		/* Disable charging if highest value selected by */
		spin_lock_irqsave(&chip->ibat_change_lock, flags);
	...
	pr_smb("overheat %d, warm %d, cold %d, low = %d deciDegC, high = %d deciDegC\n",
			chip->bat_is_overheat, chip->bat_is_warm, chip->bat_is_cold,
	...
	if (chip->batt_present ^ batt_present) {
...
#ifdef CONFIG_TCT_8X16_ALTO45
		if ((chip->cfg_cold_bat_decidegc || chip->cfg_overheat_bat_decidegc
#else
		if ((chip->cfg_cool_bat_decidegc
			...
			if (qpnp_adc_tm_channel_measure(chip->adc_tm_dev,
				...
				pr_err("request ADC error\n");
#ifdef CONFIG_TCT_8X16_ALTO45
		} else if ((chip->cfg_cold_bat_decidegc || chip->cfg_overheat_bat_decidegc
#else
...
static int qpnp_lbc_probe(struct spmi_device *spmi)
{
...
#ifdef CONFIG_TCT_8X16_ALTO45
	if ((chip->cfg_cold_bat_decidegc || chip->cfg_warm_bat_decidegc || chip->cfg_overheat_bat_decidegc)
#else
	if ((chip->cfg_cool_bat_decidegc || chip->cfg_warm_bat_decidegc)
#endif
			&& chip->bat_if_base) {
	...
	pr_info("Probe chg_dis=%d bpd=%d usb=%d batt_pres=%d batt_volt=%d soc=%d\n",
            ...
            qpnp_lbc_masked_write(chip, chip->chgr_base + CHGR_LED,
		    	0xff,0x01);/*[BUGFIX]-Add by TCTNB.XQJ,FR-723030 2014/07/09,set charger led register*/
