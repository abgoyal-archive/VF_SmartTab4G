...
//[PLATFORM]-Modify-BEGIN by TCTSZ.Leo.guo, 2014/07/10, added for external ntc support
#if (defined CONFIG_QPNP_SMB1360_EXTERNAL_NTC_SUPPORT)
#include <linux/wakelock.h>
#include <linux/workqueue.h>
#include <linux/alarmtimer.h>
#endif
//[PLATFORM]-Modify-END by TCTSZ.Leo.guo
//[PLATFORM]-Modify-BEGIN by TCTSZ.Leo.guo, 2014/05/27, added for support smb1360 version es
#if (defined CONFIG_QPNP_SMB1360_ES_SUPPORT_FOR_POP8LTE)
#include <linux/uaccess.h>
#include <linux/qpnp/qpnp-adc.h>
#endif
//[PLATFORM]-Modify-END by TCTSZ.Leo.guo
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/06/10, added for debug message.
...
/*[BUGFIX] MOD begin by TCTSZ.Leo.guo, 2014/01/01 PR-720838 The OTG function does not work */
#define OTG_CURRENT_MASK		SMB1360_MASK(4, 3)
#define OTG_CURRENT_550MA		0x01
#define OTG_CURRENT_950MA		0x02
#define OTG_CURRENT_1500MA	0x03
#define OTG_CURRENT_SHIFT		3
/*[BUGFIX] MOD end by TCTSZ.Leo.guo*/
#define RECHG_MV_MASK				SMB1360_MASK(6, 5)
#define RECHG_MV_SHIFT			5
#define OTG_CURRENT_MASK		SMB1360_MASK(4, 3)
#define OTG_CURRENT_SHIFT		3

#define CFG_TERM_CHG_REG		0x01

#define CFG_BATT_CHG_ICL_REG		0x05
#define AC_INPUT_ICL_PIN_BIT		BIT(7)
#define AC_INPUT_PIN_HIGH_BIT		BIT(6)
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/06/13,merge Qualcomm's patch, add workaround for USB100 issue.
...
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/06/14, Fixed smb1360 version es 2.1 battery id issue.
...
/*[BUGFIX] MOD begin by TCTSZ.Leo.guo, 2014/08/01 PR-720838 The OTG function does not work */
#define OTG_UVLO_VOLTAGE_REG	0x12
#define OTG_UVLO_VOLTAGE_MASK	SMB1360_MASK(4, 2)
#define OTG_UVLO_VOL_275		0x02
#define OTG_UVLO_VOL_SHIFT		2
/*[BUGFIX] MOD end by TCTSZ.Leo.guo */
...
#define BATT_CHG_FULL_BIT		BIT(7)
...
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/02, Fixed smb1360 version es 2.1 battery curve adjust issue.
#define SHDW_FG_BATT_SOC		0x66
//[PLATFORM]-Modify-BEGIN by TCTSZ.Leo.guo
...
#define BATTERY_CUTOFF_VOLTAGE	3300000   /* unit uV */
#define BATTERY_LOW_VOLTAGE		3450000   /* unit uV */
#define BATTERY_DELTA_VOLTAGE		50000   /* unit uV */

//[PLATFORM]-Modify-BEGIN by TCTSZ.Leo.guo, 2014/07/10, added for external ntc support
#if (defined CONFIG_QPNP_SMB1360_EXTERNAL_NTC_SUPPORT)
#define DEFAULT_COLD_TEMP		100
#define DEFAULT_HOT_TEMP		450
#define DEFAULT_LIMIT_VOLTAGE	4100
#endif
//[PLATFORM]-Modify-END
...
};//refered to device tree set

struct smb1360_register_status{
	u8 addr;
	u8 value;
};
#endif
//[PLATFORM]-Modify-END by TCTSZ.Leo.guo, 2014/05/04
...
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/02, Fixed smb1360 version es 2.1 battery curve adjust issue.
#if (defined CONFIG_QPNP_SMB1360_ES_BATTERY_CURVE_ADJUST_FOR_POP8LTE)
	bool			batt_low;
#endif
//[PLATFORM]-Modify-END by TCTSZ.Leo.guo
...
//[PLATFORM]-Modify-BEGIN by TCTSZ.Leo.guo, 2014/07/10, added for external ntc support
	...
	int			vbat_limit_mv;
	struct mutex			monitor_status_lock;
	enum battery_health_type bat_health;
	struct delayed_work		monitor_work;
	struct wake_lock	monitor_wake_lock;
#endif
#if (defined CONFIG_QPNP_SMB1360_CHARGING_SUSPEND)
	ktime_t			last_poll;
...
static int smb1360_get_prop_batt_capacity(struct smb1360_chip *chip)
{
	...
	u8 reg;
	u32 temp = 0;
//[PLATFORM]-Modify-BEGIN by TCTSZ.Leo.guo, 2014/06/16, added for support smb1360 version es.
#if (defined CONFIG_QPNP_SMB1360_ES_SUPPORT_FOR_POP8LTE)
	union power_supply_propval ret = {0,};
	if (chip->revision == SMB1360_REV_1) {
		if (chip->bms_psy) {
			...
			if (soc == 0) {
				if(!chip->usb_present){
					INFO("smb1360 version: %d, battery status: maybe i2c error.", chip->revision);
				}
				return soc;
			}
...
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/28, Fixed smb1360 version es 3.0 termination current issue.
#if (defined CONFIG_QPNP_SMB1360_TERMINATION_CURRENT_CTL)
	if(chip->batt_full && soc != 100){
		pr_warn("battery capacity changed.\n");
		if(chip->usb_present && (chip->charging_disabled_status & TERMINATION)){
			rc = smb1360_charging_disable(chip, TERMINATION, false);
			if (rc < 0)
				pr_err("Unable to enable charging rc=%d\n", rc);
			pr_warn("battery capacity changed, re-enable charging\n");
		}
		else{
			rc = smb1360_masked_write(chip, BATT_CHG_FLT_VTG_REG, BATT_CHG_FULL_BIT, 0);
			if (rc)
				dev_err(chip->dev, "Couldn't write to SOC_DELTA_REG rc=%d\n", rc);
			chip->batt_full = false;
			pr_warn("SMB1360: set capacity status not full.\n");
		}
	}
#endif
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/29, Fixed smb1360 version es 3.0 battery curve adjust issue.
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/28, Fixed smb1360 version es 3.0 termination current issue.
#if (defined CONFIG_QPNP_SMB1360_TERMINATION_CURRENT_CTL)
	return chip->batt_full ? 100 : bound(soc, 0, 99);
#else
...
//[PLATFORM]-Modify-BEGIN by TCTSZ.Leo.guo, 2014/07/10, added for external ntc support
#if (defined CONFIG_QPNP_SMB1360_CHARGING_SUSPEND)
#define BATT_MONOTOR_POLL_PERIOD_MIN	15*60
static void smb1360_batt_monitor_set_alarm(struct smb1360_chip *chip,
					  int seconds)
{
	alarm_start(&chip->monitor_alarm,
		    ktime_add(chip->last_poll, ktime_set(seconds, 0)));
}

static enum alarmtimer_restart smb1360_monitor_alarm_callback(
	struct alarm *alarm, ktime_t now)
{
	struct smb1360_chip *chip =
		container_of(alarm, struct smb1360_chip, monitor_alarm);
	if(!wake_lock_active(&chip->monitor_wake_lock)){
		pr_debug("%d battery wake up lock .\n", __LINE__);
...
#if (defined CONFIG_QPNP_SMB1360_EXTERNAL_NTC_SUPPORT)
static int
smb1360_batt_health_get(int temperature)
{
	if( temperature <= 2)
		return BATTERY_HEALTH_COLD;
	else if( temperature > 2 && temperature <= 450)
		return BATTERY_HEALTH_GOOD;
	else if ( temperature > 450 && temperature <= 550)
		return BATTERY_HEALTH_WARM;
	else
		return BATTERY_HEALTH_OVERHEAT;
}

#define BATT_MONOTOR_POLL_PERIOD_MS 	10000
static void
smb1360_batt_monitor_work(struct work_struct *work)
{
	int temperature, current_ua;
	enum battery_health_type bh;
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/28, Fixed smb1360 version es 3.0 termination current issue.
#if (defined CONFIG_QPNP_SMB1360_TERMINATION_CURRENT_CTL)
	int rc;
#endif
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/28
#if (defined CONFIG_QPNP_BATT_STATUS_DEBUG)
	...
	struct smb1360_chip *chip = container_of(dwork,
				struct smb1360_chip, monitor_work);

	if(!wake_lock_active(&chip->monitor_wake_lock)){
		pr_debug("%d battery wake up lock .\n", __LINE__);
		wake_lock(&chip->monitor_wake_lock);
	}
	mutex_lock(&chip->monitor_status_lock);
	...
	current_ua = smb1360_get_prop_current_now(chip);
	bh = smb1360_batt_health_get(temperature);
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/28, Fixed smb1360 version es 3.0 termination current issue.
#if (defined CONFIG_QPNP_SMB1360_TERMINATION_CURRENT_CTL)
	if(chip->usb_present && chip->batt_full
		&& (!(chip->charging_disabled_status & TERMINATION))
		&& (voltage >= chip->vfloat_mv * 1000)
		&& (current_ua + chip->iterm_ma * 1000) >= 0){
		rc = smb1360_charging_disable(chip, TERMINATION, true);
		if (rc < 0)
			pr_err("Unable to disable charging rc=%d\n", rc);
		pr_warn("current ternimation:%d mA, disable charging\n", chip->iterm_ma);
	}
#endif
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/28
	if (chip->bat_health != bh){
		switch(bh){
			case BATTERY_HEALTH_GOOD:
				...
				{
					pr_warn("battery headth good, re-enable charging, set float voltage:%d, level: %d\n",
						chip->vfloat_mv, THERMAL_MITIGATION_1500);
					/* set the limit current */
					smb1360_system_temp_level_set(chip, THERMAL_MITIGATION_1500);
					/* set the limit voltage */
					smb1360_float_voltage_set(chip, chip->vfloat_mv);
					power_supply_set_present(chip->usb_psy, true);
				}
				break;
			case BATTERY_HEALTH_WARM:
				if(chip->charging_disabled_status & THERMAL)
				{
					pr_warn("battery headth warm, re-enable charging, set float voltage:%d, level:%d\n",
						chip->vbat_limit_mv, THERMAL_MITIGATION_600);
					/* set the limit current */
					smb1360_system_temp_level_set(chip, THERMAL_MITIGATION_600);
					/* set the limit voltage */
					smb1360_float_voltage_set(chip, chip->vbat_limit_mv);
					power_supply_set_present(chip->usb_psy, true);
				}
				break;
			case BATTERY_HEALTH_OVERHEAT:
			case BATTERY_HEALTH_COLD:
				pr_warn("battery headth overheat or cold, disable charging!\n");
				smb1360_system_temp_level_set(chip, THERMAL_MITIGATION_0);
	...
	pr_debug("battery health: %d\n", bh);
	chip->bat_health = bh;
/*[BUGFIX] MOD begin by TCTSZ.Leo.guo, 2014/08/16, Add workaround for battery discharge issue */
#if defined(CONFIG_TCT_8X16_POP8LTE)
	if (voltage < BATTERY_CUTOFF_VOLTAGE && pm_power_off != NULL){
		timeout ++;
		INFO("SMB1360: detected %d times cutoff voltage, forced shutdown at battery voltage %d", timeout, voltage);
		if(timeout > 3)
			pm_power_off();
	}
#endif
/*[BUGFIX] MOD begin by TCTSZ.Leo.guo, 2014/08/16 */
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/29, Fixed smb1360 version es 3.0 battery curve adjust issue.
#if (defined CONFIG_QPNP_SMB1360_ES_BATTERY_CURVE_ADJUST_FOR_POP8LTE)
	if(voltage < BATTERY_LOW_VOLTAGE){
		pr_warn("battery voltage low than BATTERY_LOW_VOLTAGE!\n");
		chip->batt_low = true;
		power_supply_changed(&chip->batt_psy);
	}

	if((chip->empty_soc || chip->batt_low) && (voltage > BATTERY_LOW_VOLTAGE)){
		chip->batt_low = chip->empty_soc = false;
		pr_warn("SMB1360: clear battery low status.\n");
	}
#endif
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/09
#if (defined CONFIG_QPNP_BATT_STATUS_DEBUG)
	chip->index++;
	do_gettimeofday(&now);
	rtc_time_to_tm(now.tv_sec, &tm);
	INFO("%-8d    %02d:%02d:%02d    %8d    %3d%%    %3dC    %8d",chip->index, tm.tm_hour, tm.tm_min, tm.tm_sec, voltage, capacity, temperature, current_ua);
#endif

	power_supply_changed(&chip->batt_psy);

	mutex_unlock(&chip->monitor_status_lock);
	schedule_delayed_work(&chip->monitor_work,
		msecs_to_jiffies(BATT_MONOTOR_POLL_PERIOD_MS));

#if (!defined CONFIG_QPNP_SMB1360_CHARGING_SUSPEND)
	if(!chip->usb_present && wake_lock_active(&chip->monitor_wake_lock))
#endif
	{
		pr_debug("%d battery wake up unlock .\n", __LINE__);
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL:
		...
		break;
//[PLATFORM]-Modify-BEGIN by TCTSZ.Leo.guo, 2014/07/18, added for support smb1360 version es
#if (defined CONFIG_QPNP_SMB1360_ES_SUPPORT_FOR_POP8LTE)
	case POWER_SUPPLY_PROP_STATUS:
		pr_debug("do nothing, just remove warning.");
		break;
#endif
//[PLATFORM]-Modify-END by TCTSZ.Leo.guo
	default:
...
#if (defined CONFIG_QPNP_SMB1360_ES_BATTERY_CURVE_ADJUST_FOR_POP8LTE)
	if (rt_stat) {
		pr_warn_ratelimited("smb1360 batter voltage is low.\n");
		chip->batt_low = true;
...
#if (defined CONFIG_QPNP_SMB1360_TERMINATION_CURRENT_CTL)
	int rc;
	if(chip->charging_disabled_status & TERMINATION){
		rc = smb1360_charging_disable(chip, TERMINATION, false);
		if (rc < 0)
			pr_err("Unable to disable charging rc=%d\n", rc);
		pr_warn("usb plug-out,clear disable charging status.\n");
	}
#endif
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo
...
{
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/08, Fixed smb1360 version es 2.1 battery curve adjust issue.
#if defined(CONFIG_QPNP_SMB1360_ES_BATTERY_CURVE_ADJUST_FOR_POP8LTE)
	int voltage = smb1360_get_prop_voltage_now(chip);
	if(voltage > BATTERY_LOW_VOLTAGE){
		pr_warn("battery voltage more than BATTERY_LOW_VOLTAGE!\n");
		return 0;
	}
#endif
//[PLATFORM]-Add-END by TCTSZ.Leo.guo
...
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/29, Fixed smb1360 version es 3.0 battery curve adjust issue.
	...
	if (rt_stat){
		pr_warn("SOC is 100\n");
		rc = smb1360_masked_write(chip, BATT_CHG_FLT_VTG_REG, BATT_CHG_FULL_BIT, BATT_CHG_FULL_BIT);
		if (rc)
			dev_err(chip->dev, "Couldn't write to SOC_DELTA_REG rc=%d\n", rc);
		chip->batt_full = true;
		pr_warn("SMB1360: set capacity status full.\n");
	}
#endif
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo
	return 0;
...
static irqreturn_t smb1360_stat_handler(int irq, void *dev_id)
{
	...
	for (i = 0; i < ARRAY_SIZE(handlers); i++) {
		...
		for (j = 0; j < ARRAY_SIZE(handlers[i].irq_info); j++) {
			if((handlers[i].irq_info[j].name != NULL) && !strcmp(handlers[i].irq_info[j].name,  "batt_id_complete")){
				triggered = handlers[i].val
					   & (BATT_ID_LATCHED_MASK << (j * BITS_PER_IRQ));
				rt_stat = handlers[i].val
					& (BATT_ID_STATUS_MASK << (j * BITS_PER_IRQ));
...
{
	return  ((reg[3] & 0xF) << 24) | (reg[2] << 16) | (reg[1] << 8) | reg[0];
}
...
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/29, Fixed smb1360 version es 3.0 battery curve adjust issue.
	...
	if (reg & BATT_CHG_FULL_BIT){
		chip->batt_full = true;
		pr_warn("SMB1360: get capacity status is full.\n");
	}
#endif
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo
...
/*[BUGFIX] MOD begin by TCTSZ.Leo.guo, 2014/08/01 PR-720838 The OTG function does not work */
#if defined(CONFIG_TCT_8X16_POP8LTE)
	/* set otg battery current */
	rc = smb1360_masked_write(chip, CFG_BATT_CHG_REG,
						OTG_CURRENT_MASK, OTG_CURRENT_1500MA << OTG_CURRENT_SHIFT);
	if (rc < 0) {
		dev_err(chip->dev,
		"Couldn't set otg current,rc = %d\n", rc);
		 return rc;
	}

	rc = smb1360_masked_write(chip, OTG_UVLO_VOLTAGE_REG,
						OTG_UVLO_VOLTAGE_MASK, OTG_UVLO_VOL_275 << OTG_UVLO_VOL_SHIFT);
	if (rc < 0) {
		dev_err(chip->dev,
		"Couldn't set otg uvlo voltage,rc = %d\n", rc);
		 return rc;
	}
#endif
/*[BUGFIX] MOD end by TCTSZ.Leo.guo */
	...
	if (chip->iterm_ma != -EINVAL) {
		if (chip->iterm_disabled) {
			...
			rc = smb1360_masked_write(chip, CFG_TERM_CHG_REG,
						CHG_ITERM_MASK, reg);
			if (rc) {
				dev_err(chip->dev,
					"Couldn't set iterm 2 rc = %d\n", rc);
				return rc;
			}
...
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/08/18,Fix smb1360 termination current issue.
			rc = smb1360_masked_write(chip, CFG_CHG_FUNC_CTRL_REG,
					CHG_RECHG_THRESH_FG_SRC_BIT, CHG_RECHG_THRESH_FG_SRC_BIT);
//[PLATFORM]-Add-END by TCTSZ.Leo.guo, 2014/08/18
			if (rc) {
				dev_err(chip->dev,
					"Couldn't enable cfg iterm rc = %d\n", rc);
				return rc;
			}
		}
...
//[PLATFORM]-Modify-BEGIN by TCTSZ.Leo.guo, 2014/07/10, added for external ntc support
	...
	if (rc < 0)
		chip->cold_bat_decidegc = DEFAULT_COLD_TEMP;

	rc = of_property_read_u32(node, "qcom,hot-bat-decidegc",
						&chip->hot_bat_decidegc);
	if (rc < 0)
		chip->hot_bat_decidegc = DEFAULT_HOT_TEMP;

	rc = of_property_read_u32(node, "qcom,vbat-limit-mv",
						&chip->vbat_limit_mv);
	if (rc < 0)
		chip->vbat_limit_mv = DEFAULT_LIMIT_VOLTAGE;
#endif
//[PLATFORM]-Modify-END by TCTSZ.Leo.guo
...
//[PLATFORM]-Modify-BEGIN by TCTSZ.Leo.guo, 2014/07/10, added for external ntc support.
#if (defined CONFIG_QPNP_SMB1360_CHARGING_SUSPEND)
	chip->last_poll = ktime_get_boottime();
	alarm_init(&chip->monitor_alarm, ALARM_BOOTTIME, smb1360_monitor_alarm_callback);
#endif
#if (defined CONFIG_QPNP_SMB1360_EXTERNAL_NTC_SUPPORT)
	mutex_init(&chip->monitor_status_lock);
	wake_lock_init(&chip->monitor_wake_lock, WAKE_LOCK_SUSPEND,
			"smb1360-monitor");
	INIT_DELAYED_WORK(&chip->monitor_work, smb1360_batt_monitor_work);
	schedule_delayed_work(&chip->monitor_work,
		msecs_to_jiffies(BATT_MONOTOR_POLL_PERIOD_MS));
#endif
//[PLATFORM]-Modify-END by TCTSZ.Leo.guo
...
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/10, added for external ntc support.
#if (defined CONFIG_QPNP_SMB1360_EXTERNAL_NTC_SUPPORT)
...
//[PLATFORM]-Add-BEGIN by TCTSZ.Leo.guo, 2014/07/10, added for external ntc support.
	...
	wake_lock_destroy(&chip->monitor_wake_lock);
	mutex_destroy(&chip->monitor_status_lock);
#endif
//[PLATFORM]-Modify-END by TCTSZ.Leo.guo
	regulator_unregister(chip->otg_vreg.rdev);
	power_supply_unregister(&chip->batt_psy);
...
#if (defined CONFIG_QPNP_SMB1360_CHARGING_SUSPEND)
	chip->last_poll = ktime_get_boottime();
	smb1360_batt_monitor_set_alarm(chip, BATT_MONOTOR_POLL_PERIOD_MIN);
#endif
//[PLATFORM]-Modify-END by TCTSZ.Leo.guo
