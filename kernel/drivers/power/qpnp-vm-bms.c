...
/* [PLATFORM]-Mod-BEGIN by TCTNB.FLF, FR-675806, 2014/05/16, add battery paramters */
//[PLATFORM]-Modify-BEGIN by TCTSZ.Leo.guo, 2014/05/27, added for support smb1360 version es
#if	(defined CONFIG_TCT_8X16_ALTO45 || defined CONFIG_TCT_8X16_POP8LTE)
...
static int get_batt_therm(struct qpnp_bms_chip *chip, int *batt_temp)
{
	int rc;
	struct qpnp_vadc_result result;
//[PLATFORM]-Modify-BEGIN by TCTSZ.Leo.guo, 2014/06/12, modify MMI version build warning.
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.FLF, FR-675806, 2014/05/16, add battery paramters */
	...
	if (chip->dt.batt_type == BATT_TCT_4V35_2000MAH_ALTO45) {
		battery_id = read_battery_id(chip);
		if (battery_id < 0) {
			pr_err("cannot read battery id err = %lld\n", battery_id);
			return battery_id;
		}
		pr_debug("battery_id = %lld\n", battery_id);
		...
		if (battery_id>200000 && battery_id<400000)		//BYD
			batt_data = &tct_4v35_2000mah_byd_alto45;
		else if (battery_id>1100000 && battery_id<1300000)
			batt_data = &tct_4v35_2000mah_scud_alto45;	//SCUD
		else
			batt_data = &tct_4v35_2000mah_byd_alto45;
	} else {
		battery_id = read_battery_id(chip);
...
//[PLATFORM]-Modify-BEGIN by TCTSZ.Leo.guo, 2014/05/27, added for support smb1360 version es
#elif (defined CONFIG_TCT_8X16_POP8LTE)
	if (chip->dt.batt_type == BATT_TCT_4V35_4060MAH_POP8S) {
		batt_data = devm_kzalloc(chip->dev,
				sizeof(struct bms_battery_data), GFP_KERNEL);
		if (!batt_data) {
			pr_err("Could not alloc battery data\n");
		...
		batt_data = &tct_4v35_4060mah_scud_pop8s;
		batt_data->max_voltage_uv = -1;
		batt_data->cutoff_uv = -1;
		batt_data->iterm_ua = -1;
	} else {
...
/* [PLATFORM]-Mod-END by TCTNB.FLF */
//[PLATFORM]-Modify-BEGIN by TCTSZ.Leo.guo, 2014/05/27, added for support smb1360 version es
	if (batt_data->ibat_acc_lut != NULL){
...
//[PLATFORM]-Add-BEGIN by chenhui, 2014/08/06, POP8LTE for" don't request irq when use smb1360
#ifdef CONFIG_TCT_8X16_POP8LTE
	if(0)
	{
#endif
//[PLATFORM]-Add-END by chenhui, 2014/08/06
