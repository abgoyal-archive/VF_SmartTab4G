...
static struct scatterlist* g_mqrq_cur = NULL;
static struct scatterlist* g_mqrq_pre = NULL;
...
retry:
		...
		if(mq->card->type != MMC_TYPE_SD){
			...
			if(g_mqrq_cur == NULL){
				g_mqrq_cur = mmc_alloc_sg(host->max_segs, &ret);
				if (ret){
					kfree(g_mqrq_cur);
					g_mqrq_cur = NULL;
					goto cleanup_queue;
			...
			mqrq_cur->sg = g_mqrq_cur;

			if(g_mqrq_pre == NULL){
				g_mqrq_pre = mmc_alloc_sg(host->max_segs, &ret);
				if (ret){
					kfree(g_mqrq_pre);
					g_mqrq_pre = NULL;
					goto cleanup_queue;
