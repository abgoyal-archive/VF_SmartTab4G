...
u8 *config;
u8 config_hi_sense[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
                = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
u8 config_nor_sense[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
                = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
...
//static struct wake_lock tp_wake_lock;	/*ADD by TCTSZ.WH,2014-6-16,In case of PAD enter sleep before run tp irq handler.*/
...
extern int g_tp_cfg_ver; //modify by weihong.chen 2014/06/16
...
if (g_wakeup_gesture)
{
    if (DOZE_ENABLED == doze_status)
    {
	msleep(10);	/*ADD by TCTSZ-WH,2014-6-24. Avoid read before i2c bus resume.*/
        ...
        if (ret > 0)
        {
            ...
            {
                doze_status = DOZE_WAKEUP;
/*ADD Begin by TCTSZ-WH,2014-6-17,Double click to unlock screen. Write 1 to light up, 2 to unlock.*/
		...
		{
			GTP_INFO("Double click to unlock the screen!");
			input_report_key(ts->input_dev, KEY_UNLOCK, 1);
			input_sync(ts->input_dev);
			input_report_key(ts->input_dev, KEY_UNLOCK, 0);
			input_sync(ts->input_dev);
		}
/*ADD End by TCTSZ-WH,2014-6-17,Double click to unlock screen.*/
                // clear 0x814B
                doze_buf[2] = 0x00;
                gtp_i2c_write(i2c_connect_client, doze_buf, 3);
            }
...
/*[BUGFIX] Add Begin by TCTSZ.WH,2014-5-27,Add palm lock function.*/
      ...
      /*ADD Begin by TCTSZ-weihong.chen,2014-6-03,Add debug interface for input devices.*/
#ifdef CONFIG_TCT_8X16_POP8LTE
    if((finger&(1<<6)) && (0 == palm_lock_flag)&& g_palm_lock_switch)	/*MOD by TCTSZ-WH,2014-6-25, Seperate the two  switch.*/
#else
...
//    wake_lock(&tp_wake_lock);	/*ADD by TCTSZ.WH,2014-6-16,In case of PAD enter sleep before run tp irq handler.*/
...
/*MOD Begin by TCTSZ-WH,2014-6-27. Power on/off if gestured mode is disabled.*/
if(!g_wakeup_gesture)
{
	...
	hall->tp_set_sensitivity(gpio_get_value(hall->irq_gpio));	/*Mod by TCTSZ-WH,2014-7-4. Send config when resume.*/
#if 0
	ret = gtp_send_cfg(ts->client);
        if (ret > 0)
        {
...
/*MOD End by TCTSZ-WH,2014-6-27. Power on/off if gestured mode is disabled.*/
#if GTP_DRIVER_SEND_CFG
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    u8 cfg_info_group6[] = CTP_CFG_GROUP6_HI_SENSE;
...
//    memset(&config[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
    memset(&config_nor_sense[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
    memset(&config_hi_sense[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
    memcpy(&config_nor_sense[GTP_ADDR_LENGTH], send_cfg_buf[sensor_id], ts->gtp_cfg_len);
    memcpy(&config_hi_sense[GTP_ADDR_LENGTH], send_cfg_buf[5], ts->gtp_cfg_len);

    config = config_nor_sense;
#if GTP_CUSTOM_CFG
...
input_mt_init_slots(ts->input_dev, 10, 0);
    ...
    input_set_capability(ts->input_dev, EV_KEY, KEY_UNLOCK);	/*ADD by TCTSZ-WH,2014-6-17,Double click to unlock screen.*/
#endif
...
/*ADD Begin by TCTSZ.WH,2014-6-18*/
static void gt9xx_set_sensitivity(int status)	/*MOD by TCTSZ-WH,2014-6-23,Improve the independence between the two modules.*/
{
	s32 ret = 0;

        GTP_INFO("enter %s\n",__func__);

	if(!status)	//hall closed
	{
		GTP_INFO("Cover closed,Set high sensitivity");
		config = config_hi_sense;
	}
	else
	{
		GTP_INFO("Cover open,Set normal sensitivity");
		config = config_nor_sense;
	}
...
printk(">>-- %s line:%d, ret=%d \n", __func__, __LINE__, ret);
    ...
    /*ADD Begin by TCTSZ-WH,2014-6-18. Init function, run once in probe()*/
    hall->tp_set_sensitivity = gt9xx_set_sensitivity;
    hall->tp_set_sensitivity(gpio_get_value(hall->irq_gpio));		//Put behind gtp_init_panel(ts)
    /*ADD End by TCTSZ-WH,2014-6-18.*/
	...
	/*MOD by TCTSZ.WH,2014-6-19, Initialize wakelock before used in irq handler.*/
//    wake_lock_init(&tp_wake_lock,WAKE_LOCK_SUSPEND,"tp_wake_lock");	/*ADD by TCTSZ.WH,2014-6-16,In case of PAD enter sleep before run tp irq handler.*/
...
/*ADD Begin by TCTSZ.WH,2014-6-16,Enable tp wakeup.*/
	if(g_wakeup_gesture)
	{
		device_init_wakeup(&ts->client->dev,1);
		enable_irq_wake(ts->client->irq);
	}
/*ADD End by TCTSZ.WH,2014-6-16,Enable tp wakeup.*/
    }
...
static void gtp_esd_check_func(struct work_struct *work)
{
    ...
    if (ts->gtp_is_suspend || ts->enter_update)		/*MOD by TCTSZ-WH,2014-7-19.Avoid enter esd_check_func when updating FW.*/
    {
