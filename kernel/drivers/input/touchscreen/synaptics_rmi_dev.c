...
#if GTP_ESD_PROTECT
extern void synaptics_esd_switch(struct synaptics_rmi4_data *rmi4_data, int on);
#endif
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dev_dbg(&rmidev->rmi4_data->i2c_client->dev,
			...
			__func__);
     #if GTP_ESD_PROTECT
	 if(rmidev->rmi4_data->esd_running)
     synaptics_esd_switch(rmidev->rmi4_data, SWITCH_OFF);
     #endif
	return count;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dev_dbg(&rmidev->rmi4_data->i2c_client->dev,
			...
			__func__);
     #if GTP_ESD_PROTECT
	 if((!rmidev->rmi4_data->esd_running)&&(!rmidev->rmi4_data->suspended))
     synaptics_esd_switch(rmidev->rmi4_data, SWITCH_ON);
     #endif
	return count;
}
